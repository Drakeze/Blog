...
function requireExcerptWhenPublished(
  v: { status?: string; excerpt?: string },
  ctx: z.RefinementCtx
) {
  if ((v.status ?? 'draft') === 'published' && !v.excerpt?.trim()) {
    ctx.addIssue({ code: 'custom', path: ['excerpt'], message: 'Excerpt is required to publish' });
  }
}

export const postInputSchema = postObjectSchema.superRefine(requireExcerptWhenPublished);

export const postUpdateSchema = postObjectSchema
  .partial()
  .refine((v) => Object.keys(v).length > 0, 'At least one field is required')
  .superRefine(requireExcerptWhenPublished);
