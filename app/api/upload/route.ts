...
export async function POST(req: Request) {
  ...
  if (!storageConfig.configured) {
    return NextResponse.json(
      { error: 'Image storage not configured. Set the R2 environment variables.' },
      { status: 503 }
    );
  }
  ...
  try {
    ...
    // Most common cause: the R2 key pair here is wrong/revoked (403
    // SignatureDoesNotMatch). Surface it instead of a bare 500.
